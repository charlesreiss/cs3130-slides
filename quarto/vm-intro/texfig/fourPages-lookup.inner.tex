...
\end{visibleenv}
\begin{visibleenv}<4>
    \node[fill=white,anchor=south west,xshift=-1cm] at (addrLeft.north east) {\myemph{``virtual page number''}};
\end{visibleenv}
\begin{visibleenv}<5>
...
\begin{visibleenv}<6>
    ...
    \node[fill=white,xshift=-2cm,anchor=south west] at (addrRest.north east) {\myemph{``page offset''}};
\end{visibleenv}
\end{tikzpicture}
