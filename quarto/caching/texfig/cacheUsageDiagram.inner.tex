...
\end{visibleenv}
        \node[anchor=north,align=center] at (4.5,-.5) {\myemph<5>{for all $k$:} \myemph<4>{for all $i$:} \myemph<3>{for all $j$}: $B_{ij} += \myemph<3>{A_{ik}} \times \myemph<4>{A_{kj}}$ \\ $N$ calculations for $A_{ik}$ \\ $1$ for $A_{kj}$, $B_{ij}$};
\begin{visibleenv}<3>
