...
\node[anchor=south west,align=left] (example1) at ([yshift=0.5cm]cache1.north west) {
    ...
    \only<0>{???}\only<7->{\itag{001}} &
    \only<0>{???}\only<4->{\iindex{11}} &
    \only<0>{???}\only<2->{\offset{1}} \\
    \myemph<2>{2 byte blocks}, \myemph<4>{8 sets} &
    \only<0>{???}\only<7->{\itag{00}} &
    \only<0>{???}\only<5->{\iindex{111}} &
    \only<0>{???}\only<2->{\offset{1}} \\
    \myemph<3>{4 byte blocks}, \myemph<4>{2 sets} &
    \only<0>{???}\only<7->{\itag{001}} &
    \only<0>{???}\only<4->{\iindex{1}} &
    \only<0>{???}\only<3->{\offset{11}}\\
    \end{tabular}
};
\tikzset{
