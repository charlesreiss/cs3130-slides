...
\node[draw,anchor=north west,align=left] at (noteLoc) {
    ...
    \draw[very thick,red,->] ([yshift=-1.0cm,xshift=-1cm]exCode.north) -- ++(1cm,0cm) node[right] {fault!};
\end{visibleenv}
\begin{visibleenv}<3>
\node[inner sep=0.25mm,draw=red,ultra thick,fit=(pt-3-2) (pt-3-3)] {};
\node[draw,anchor=north west,align=left] at (noteLoc) {
...
\draw[very thick,red,->] ([yshift=-1.0cm,xshift=-1cm]exCode.north) -- ++(2cm,0cm)
        -- ++(0cm,.4cm) node[midway,right] {restarted} -- ++(-2cm,0cm);
\end{visibleenv}
\end{tikzpicture}
