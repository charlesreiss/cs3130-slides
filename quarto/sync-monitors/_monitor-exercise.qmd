...
(one of many possible solutions) <br> Assuming ConsumeTwo is <em>in addition to</em> Consume (using two CVs): 
```
Produce() {
  pthread_mutex_lock(&lock);
  buffer.enqueue(item);
...
(one of many possible solutions) <br> Assuming ConsumeTwo is <em>in addition to</em> Consume (using one CV): 
```
Produce() {
  pthread_mutex_lock(&lock);
  buffer.enqueue(item);
