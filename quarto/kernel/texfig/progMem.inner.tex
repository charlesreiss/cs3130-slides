...
\node[mybox, minimum height=.6cm, below=.7cm of kernel] (stack) {Stack};
...
\node[mybox, minimum height=.6cm, below=.7cm of kernel] (stack) {Stack};
\node[mybox, minimum height=1.1cm, below=.3cm of stack] (heap) {Heap / other dynamic};
\node[mybox, minimum height=.6cm, below=0mm of heap] (data) {Writable data};
\node[mybox, minimum height=.55cm, below=0mm of data] (sdata) {Code + Constants};
\coordinate (memBottom) at ($(sdata.south east) + (0mm, -2mm)$);
\begin{pgfonlayer}{bg}
\draw[pattern=north west lines, pattern color=black!40!white] (kernel.north west) rectangle (memBottom);
\end{pgfonlayer}
