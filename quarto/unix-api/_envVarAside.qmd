...
### aside: environment variables (1)  {.smaller}



* key=value pairs associated with every process:
 
<pre><code>$ <em>printenv</em>
...
### aside: environment variables (2)  {.smaller}



* environment variable library functions: 

   * <code>getenv("KEY")</code> $\\rightarrow{}$ <i>value</i>
   * <code>putenv("KEY=<i>value</i>")</code> (sets KEY to <i>value</i>)
   * <code>setenv("KEY", "value")</code> (sets KEY to <i>value</i>)
...
### aside: environment variables (3)  {.smaller}


* interpretation up to programs, but common ones… 
<hr class="vspace" />
* <code>PATH=/bin:/usr/bin</code>  

   * to run a program ‘foo’, look for an executable in <code>/bin/foo</code>, then <code>/usr/bin/foo</code>

* <code>HOME=/zf14/cr4bd</code>  

   * current user's home directory is ‘/zf14/cr4bd’

* <code>TERM=screen-256color</code>  
