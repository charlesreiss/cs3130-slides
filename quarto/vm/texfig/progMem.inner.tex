...
\node[mybox, minimum height=.6cm, below=1cm of stack] (heap) {Heap / other dynamic};
\node[mybox, minimum height=.6cm, below=0mm of heap] (data) {Writable data};
\node[mybox, minimum height=.6cm, below=0mm of data] (sdata) {vim (Code + Constants)};
...
\node[mybox, minimum height=.6cm, below=1cm of kernel] (stack) {Stack};
\node[mybox, minimum height=1.3cm, below=.3cm of stack] (heap) {Heap / other dynamic};
\node[mybox, minimum height=.6cm, below=0mm of heap] (data) {Writable data};
\node[mybox, minimum height=.6cm, below=0mm of data] (sdata) {vim (Code + Constants)};
\coordinate (memBottom) at ($(sdata.south east) + (0mm, -2mm)$);
\begin{pgfonlayer}{bg}
