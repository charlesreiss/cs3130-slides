...
\begin{itemize}
\item 5-bit virtual addresses, 6-bit physical addresses, 8-byte pages
\item (virtual addresses) \small \myemph<2>{\tt 0x18} = \exAnswer<2->{\tt 00}; {\tt 0x03} = \exAnswer<3->{0x4A}; {\tt 0x0A} = \exAnswer<4->{\tt 0xDC}; {\tt 0x13} = \exAnswer<5->{fault} 
\end{itemize}
\begin{tikzpicture}
\matrix[tight matrix,anchor=north west,
    nodes={text width=2cm,minimum height=0.6cm},
...
\matrix[tight matrix,anchor=north west,
    ...
    physical addresses \& bytes \\
    0x00-3 \& \emphAns<2>{00} 11 22 33 \\
    0x04-7 \& 44 55 66 77 \\
    0x08-B \& 88 99 AA BB \\
    0x0C-F \& CC DD EE FF \\
    0x10-3 \& 1A 2A 3A \emphAns<3>{4A} \\
    0x14-7 \& 1B 2B 3B 4B \\
    0x18-B \& 1C 2C 3C 4C \\
    0x1C-F \& 1C 2C 3C 4C \\
};
