...
\begin{frame}[fragile,label=dfLimit2]{data flow model and limits (1)}
    \begin{tikzpicture}
        \tikzset{
        ...
        \begin{visibleenv}<all:1>
            \node[anchor=west,align=left,font=\small\tt] at ([xshift=1cm,yshift=-.5cm]i0.east) {
loop2: \\
\hspace{1cm}    addq \%rbx, \%rcx \\
\hspace{1cm}   addq \%r8, \%r9 \\
\hspace{1cm}   decq \%rax \\
\hspace{1cm}   jge loop2 \\
};
        \end{visibleenv}
