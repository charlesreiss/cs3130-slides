...
\item Intel Haswell CPU did something similar to this
    \begin{itemize}
    \item uses Hash(bits of last several jumps), not just last jmp
    \end{itemize}
\end{itemize}
\end{frame}
...
\item 2: mistrain branch target predictor for it to jump to chosen code
    ...
    \item since only bottom bits are used, can set this up in user memory
    \end{itemize}
\item 3: have chosen code be attack code (e.g. array access)
    ...
    \end{itemize}
\item 4: run the kernel function
\end{itemize}
\end{frame}
