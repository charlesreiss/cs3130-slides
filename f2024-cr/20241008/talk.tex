...
\begin{frame}{last time}
    \begin{itemize}
    \item locality --- spatial/temporal
    \item tag/[set] index/[block] offset split
        \begin{itemize}
        \item block size = $2^{\text{offset bits}}$
        \item number of sets (rows) = $2^{\text{index bits}}$
        \item tag = leftover address bits
        \end{itemize}
    \item direct-mapped lookup
        \begin{itemize}
        \item split into tag/index/offset
        \item go to row \textit{index}
        \item check valid bit + tag
        \item if match, return block byte \textit{offset}
        \item if no match, load from memory
        \end{itemize}
    \end{itemize}
\end{frame}
