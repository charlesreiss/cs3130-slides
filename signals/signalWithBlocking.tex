...
\draw[violet,very thick,decoration={brace},decorate] ([yshift=-.5cm,xshift=.25cm]base) -- ++(0cm,-2.5cm)
    node[midway,right] {blocked};
\draw[thick,dotted] ([yshift=-2cm]base) -- ++(-.5cm,0cm) node[font=\small,align=right,left] {sigwait\\starts};
\draw[thick,dotted] ([yshift=-3cm]base) -- ++(-.5cm,0cm) node[xshift=.1cm,font=\small,align=right,left] {receive signal\\sigwait retuns};
