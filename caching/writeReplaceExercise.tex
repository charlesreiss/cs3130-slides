...
\item \myemph<3-4>{reading 1 byte from 0x52}: \only<2->{(set 1, offset 0) \textbf{write} back 0x32-0x33; \textbf{read} 0x52-0x53}
\item \myemph<5-6>{reading 1 byte from 0x50}: \only<3->{(set 0, offset 0) replace 0x30-0x31 (no write back); \textbf{read} 0x50-0x51}
\end{itemize}
\end{frame}
