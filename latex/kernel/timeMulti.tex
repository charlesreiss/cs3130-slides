...
\end{tikzpicture}
    ...
    Timeline showing loop.exe as active for some time, then nothing active, then loop.exe active
}{%
    Same diagram, with the assembly instructions listed from an infinite loop, showing a "million cycle delay" where loop.exe is inactive.
}{%
    Same diagram, but now the inactive period is filled by ssh.exe and firefox.exe.
}
\end{frame}
