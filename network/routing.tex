...
\node[aspect=3.2,draw,cloud,anchor=east,minimum width=2.5cm] (network 1) at ([xshift=-1cm,yshift=.5cm] router.west) {
    network 1
};
\node[aspect=3.2,draw,cloud,anchor=west,minimum width=2.5cm] (network 2) at ([xshift=1cm,yshift=.5cm] router.east) {
    network 2
};
\node[aspect=3.2,draw,cloud,anchor=south,minimum width=2.5cm] (network 3) at ([yshift=0.75cm] router.north) {
    network 3
};
\foreach \x in {1,2,3} { \draw[<->,very thick] (router) -- (network \x); }
\node[anchor=north,font=\small,draw,thick,inner sep=0.5mm] (table) at ([yshift=-.25cm]router.south) {
\begin{tabular}{l|l}
